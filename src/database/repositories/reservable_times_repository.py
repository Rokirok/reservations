...
def list_reservable_times() -> (list[Timeslot], list[str]):
    result = db.session.execute(text("SELECT rt.id, rt.timestamp, rt.location, rt.employee, rt.service, reservations.id IS NOT NULL as reserved FROM reservable_timeslots rt LEFT JOIN reservations ON reservations.timeslot = rt.id ORDER BY rt.timestamp DESC"))
